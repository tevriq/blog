...
- 照片类公开图片默认在发布时只生成一个最大宽度 `1280w` 的 WebP 衍生文件，原图不足 1280px 时不得放大；
- 默认不生成 `640w` 或其他多尺寸版本；
- 默认不依赖 Cloudflare Images 动态转换，以降低不可预测费用；
- 页面必须以低流量为设计目标：lazy loading、限制正文宽度、避免无意义 preload/prefetch、避免一打开就请求整篇全部图片；
- 长视频优先交给 YouTube 或其他可替换的视频平台，原始文件仍由用户掌控；
- 对外引用应优先使用用户控制的域名，而不是供应商专属域名；
- 如果承载 `blog/` 的同一个 R2 bucket 还保存私人 Obsidian/Remotely Save 数据，不得为了博客媒体而直接公开整个 bucket。
...
`docs/LOCAL_AI_IMPLEMENTATION.md` 是当前实现指引，不高于上述规范。进行本地落地开发时应同时阅读它。
